...
# treated as the forecast horizon. The /forecast endpoint requires a
# `time_column`; the task is always regression, so no `task` field is needed.
set -euo pipefail
...
curl -s -X POST https://tabh2o.h2oai.com/api/v1/forecast \
  ...
  -F time_column=date | { command -v jq >/dev/null && jq . || cat; }
